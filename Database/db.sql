...
DECLARE
BEGIN
    IF :new.EDUCATOR_NAME != :old.EDUCATOR_NAME AND
            :new.IS_INHOUSE != :old.IS_INHOUSE AND
...
ALTER TABLE SYSTEM.T_USER ADD (
  ...
  ENABLE VALIDATE,
  CONSTRAINT T_USER_U03
  UNIQUE (EMAIL)
  USING INDEX SYSTEM.T_USER_U03
  ENABLE VALIDATE);
...
ALTER TABLE SYSTEM.T_EDUCATOR ADD (
  CONSTRAINT T_EDUCATOR_PK
  PRIMARY KEY
  (PK)
  USING INDEX SYSTEM.T_EDUCATOR_PK
  ENABLE VALIDATE);
...
ALTER TABLE SYSTEM.T_USER_ABILITY_REL ADD (
  ...
  FOREIGN KEY (USER_FK) 
  REFERENCES SYSTEM.T_USER (U_ID)
  ENABLE VALIDATE);

ALTER TABLE SYSTEM.T_EDUCATOR ADD (
  CONSTRAINT T_EDUCATOR_R01 
  FOREIGN KEY (USER_FK) 
  REFERENCES SYSTEM.T_USER (U_ID)
  ON DELETE CASCADE
  ENABLE VALIDATE);
...
ALTER TABLE SYSTEM.T_EDUCATION ADD (
  ...
  FOREIGN KEY (EDUCATOR_FK) 
  REFERENCES SYSTEM.T_EDUCATOR (PK)
  ENABLE VALIDATE);
...
AS
BEGIN
    INSERT INTO T_EDUCATION(EDUCATION_SUBJECT,EDUCATION_CONTENT,PLANNED_DATE,COMPLETE_DATE,EDUCATOR_FK) VALUES(edu_subject,edu_content,TO_DATE(start_date,'DD-MM-YYYY'),TO_DATE(finish_date,'DD-MM-YYYY'),educator_id);
    is_valid := '1';
END;
/
