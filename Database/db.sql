...
CREATE OR REPLACE TRIGGER SYSTEM.TRG_AFTER_USER
AFTER INSERT
ON SYSTEM.T_USER
REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
...
CREATE OR REPLACE TRIGGER SYSTEM.TRG_BEFORE_USER_DELETE 
BEFORE DELETE
ON SYSTEM.T_USER
REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
...
BEGIN
    ...
    BEGIN
        SELECT T_UNIT.PK INTO unit_id FROM T_ROLE,T_UNIT WHERE T_ROLE.UNIT_FK = T_UNIT.PK AND T_ROLE.PK = :old.ROLE_FK;
    EXCEPTION WHEN NO_DATA_FOUND THEN
        unit_id := NULL;
    END;
    
    BEGIN
        SELECT T_DEPARTMENT.PK INTO dep_id FROM T_ROLE,T_DEPARTMENT WHERE T_ROLE.DEPARTMENT_FK = T_DEPARTMENT.PK AND T_ROLE.PK = :old.ROLE_FK;        
    EXCEPTION WHEN NO_DATA_FOUND THEN
        dep_id := NULL;
    END; 
    
    IF is_unit_manager = 'Y' THEN
        UPDATE T_UNIT SET MANAGER_ID = NULL WHERE PK = unit_id;
    ELSIF is_department_manager = 'Y' THEN
        UPDATE T_DEPARTMENT SET MANAGER_ID = NULL WHERE PK = dep_id;        
    END IF;     
END;
/
...
--  TRG_AFTER_USER  (Trigger) 
--  TRG_BEFORE_USER_DELETE  (Trigger) 
...
DROP TRIGGER SYSTEM.TRG_AFTER_USER;
...
DROP TRIGGER SYSTEM.TRG_BEFORE_USER_DELETE;
